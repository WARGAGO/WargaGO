...
class PollProvider with ChangeNotifier {
  ...
  void loadActiveOfficialPolls() {
    if (kDebugMode) {
      print('🗳️ [PollProvider] Loading active official polls...');
    }

    _pollService.streamActivePolls(pollLevel: 'official').listen(
      (polls) {
        if (kDebugMode) {
          print('✅ [PollProvider] Loaded ${polls.length} official polls');
      ...
      onError: (error) {
        if (kDebugMode) {
          print('❌ [PollProvider] Error loading official polls: $error');
          if (error.toString().contains('index')) {
            print('   🔥 INDEX ISSUE DETECTED!');
            print('   → Firestore index masih building atau belum dibuat');
            print('   → Check: https://console.firebase.google.com/project/pbl-2025-35a1c/firestore/indexes');
          }
        }
      },
    );
  ...
  void loadActiveCommunityPolls() {
    if (kDebugMode) {
      print('🗳️ [PollProvider] Loading active community polls...');
    }

    _pollService.streamActivePolls(pollLevel: 'community').listen(
      (polls) {
        if (kDebugMode) {
          print('✅ [PollProvider] Loaded ${polls.length} community polls');
          for (var poll in polls) {
            print('   - ${poll.title} (ID: ${poll.pollId})');
          }
        }
      ...
      onError: (error) {
        if (kDebugMode) {
          print('❌ [PollProvider] Error loading community polls: $error');
          if (error.toString().contains('index')) {
            print('   🔥 INDEX ISSUE DETECTED!');
            print('   → Firestore index masih building atau belum dibuat');
          }
  ...
  void loadPollForAdminMonitoring(String pollId) {
    if (kDebugMode) {
      print('👮 [PollProvider] Admin monitoring poll: $pollId');
